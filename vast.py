...
def filter_df(df):
    # We want to compute the $/GPU in multi-gpu instances
    # Extract the integer part of N and convert it to integer type
    df['N_int'] = df['N'].str.extract('(\d+)').astype(float)
    df['$/hr'] = df['$/hr'].astype(float)
    # Calculate $/GPUh
    df['$/GPUh'] = df['$/hr'] / df['N_int']

    # Remove the 'N_int' column if it's no longer needed
    df.drop('N_int', axis=1, inplace=True)

    return df

def display_table_df(rows: list, fields: typing.Tuple) -> None:
    header = [name for _, name, _, _, _ in fields]
    out_rows = [header]
    lengths = [len(x) for x in header]
    ...
    data_df = pd.DataFrame(out_rows)
    data_df.columns = data_df.iloc[0]

    data_df = data_df[1:]  # Remove the first row (which contained the header)
    # Print the DataFrame
    data_df = filter_df(data_df)
    return data_df
    #print(data_df)

def display_table_instances(rows: list, fields: typing.Tuple) -> None:
    header = [name for _, name, _, _, _ in fields]
    out_rows = [header]
    lengths = [len(x) for x in header]
...
def search__offers(args):
    ...
    if args.raw:
        ...
        return display_table_df(rows, displayable_fields)
...
def show__instances(args):
    ...
    if args.raw:
        ...
        return display_table_instances(rows, instance_fields)
